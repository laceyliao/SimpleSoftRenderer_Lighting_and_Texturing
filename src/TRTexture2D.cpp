...
namespace TinyRenderer
{
	...
	glm::vec4 TRTexture2DSampler::textureSampling_nearest(const TRTexture2D &texture, glm::vec2 uv)
	{
		...
		//       But before that, you need to map uv from [0,1]*[0,1] to [0,width-1]*[0,height-1].
		{
			...
			x = std::min(x * width, width - 1); y = std::min(y * height, height - 1);
			texture.readPixel((int)x, int(y), r, g, b, a);
		}
	...
	glm::vec4 TRTexture2DSampler::textureSampling_bilinear(const TRTexture2D &texture, glm::vec2 uv)
	{
		...
		glm::vec2 u00 = { std::floor(x),std::floor(y) };
		glm::vec2 u10 = { std::ceil(x),std::floor(y) };
		glm::vec2 u01 = { std::floor(x),std::ceil(y) };
		glm::vec2 u11 = { std::ceil(x),std::ceil(y) };

		float s = x - std::floor(x);
		float t = y - std::floor(y);

		unsigned char u0_r = 255, u0_g = 255, u0_b = 255, u0_a = 255;
		unsigned char u1_r = 255, u1_g = 255, u1_b = 255, u1_a = 255;
		unsigned char r1, r2, g1, g2, b1, b2, a1, a2;
		texture.readPixel(u00.x, u00.y, r1, g1, b1, a1);
		texture.readPixel(u10.x, u10.y, r2, g2, b2, a2);
		u0_r = r1 + s * (r2 - r1);	u0_g = g1 + s * (g2 - g1); u0_b = b1 + s * (b2 - b1); u0_a = a1 + s * (a2 - a1);

		texture.readPixel(u01.x, u01.y, r1, g1, b1, a1);
		texture.readPixel(u11.x, u11.y, r2, g2, b2, a2);
		u1_r = r1 + s * (r2 - r1);	u1_g = g1 + s * (g2 - g1); u1_b = b1 + s * (b2 - b1); u1_a = a1 + s * (a2 - a1);

		unsigned char r = 255, g = 255, b = 255, a = 255;
		r = u0_r + t * (u1_r - u0_r); g = u0_g + t * (u1_g - u0_g); b = u0_b + t * (u1_b - u0_b); a = u0_a + t * (u1_a - u0_a);
