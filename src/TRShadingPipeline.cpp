...
namespace TinyRenderer
{
	...
	//add spot light
	int TRShadingPipeline::addSpotLight(glm::vec3 pos, glm::vec3 dir, float angle, float outerangle, glm::vec3 atten, glm::vec3 color)
	{
		m_spot_lights.push_back(TRSpotLight(pos, dir, angle, outerangle, atten, color));
	...
	void TRPhongShadingPipeline::fragmentShader(const VertexData &data, glm::vec4 &fragColor)
	{
		...
		for (size_t i = 0; i < m_point_lights.size(); ++i)
		{
			...
			{
				float r = glm::length(light.lightPos - fragPos);
				float constant = light.attenuation.x, linear = light.attenuation.y, quadratic = light.attenuation.z;
				attenuation = 1.0 / (constant + linear * r + quadratic * r * r);
				...
				diffuse =  dif_color * light.lightColor / (r * r);
				diffuse *= std::max(0.0f, glm::dot(normal, lightDir));
				//phong
				/*auto reflect_lightDir = glm::normalize(glm::reflect(-lightDir, normal));
				specular =  spe_color * light.lightColor / (r * r);
				specular *= glm::pow(std::max(0.0f, glm::dot(reflect_lightDir, viewDir)), m_shininess);*/

				//blinn-phong
				auto halfwayDir = glm::normalize(lightDir + viewDir);
				specular = spe_color * light.lightColor / (r * r);
				specular *= glm::pow(std::max(0.0f, glm::dot(normal, halfwayDir)), m_shininess);
			}
	...
	//			diffuse = dif_color * intensity* light.lightColor / (r * r);
	//			diffuse *= std::max(0.0f, glm::dot(normal, lightDir));
	//			//phong
	//			/*auto reflect_lightDir = glm::normalize(glm::reflect(-lightDir, normal));
	//			specular =  spe_color * light.lightColor / (r * r);
	...
	//			//blinn-phong
	//			auto halfwayDir = glm::normalize(lightDir + viewDir);
	//			specular = spe_color * intensity * light.lightColor / (r * r);
	//			specular *= glm::pow(std::max(0.0f, glm::dot(normal, halfwayDir)), m_shininess);
	//		}
	//
	//		fragColor.x += (ambient.x + diffuse.x + specular.x) * attenuation;
